...
import { Loader2, Mail, Lock, Shield, MessageCircle, X } from "lucide-react";
        ...
        <div className="max-w-md w-full mx-auto space-y-10">
          ...
          {!showForgot ? (
            ...
            <form onSubmit={handleForgotPassword} className="space-y-6">
              <div className="space-y-4">
                <h3 className="text-xl font-black uppercase italic text-primary">Recuperar Acceso</h3>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-muted-foreground ml-1 tracking-widest">Tu Email Registrado</label>
                  <input
                    required
                    type="email"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="h-14 w-full rounded-2xl border-2 border-white/5 bg-white/[0.03] px-4 font-bold outline-none focus:border-primary/50 text-foreground"
                    placeholder="email@ejemplo.com"
                  />
                </div>
              </div>
              <div className="flex gap-3">
                <Button type="button" variant="ghost" onClick={() => setShowForgot(false)} className="flex-1 font-black uppercase text-xs">Atrás</Button>
                <Button type="submit" className="flex-[2] h-14 font-black uppercase tracking-widest" disabled={loading}>
