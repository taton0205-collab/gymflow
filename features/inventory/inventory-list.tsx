...
export function InventoryList() {
  ...
  const fetchData = async () => {
    ...
    // 2. Obtener Ganancia Real (de la tabla de pagos)
    const { data: paymentsData } = await supabase
      .from("payments")
      .select("amount, cost_basis")
      .not("notes", "is", null)
      .ilike("notes", "VENTA:%"); // Filtramos solo los pagos que sean ventas de productos
  ...
  const handleSell = async (item: any) => {
    ...
    try {
      // 1. Registrar la venta con su COSTO GUARDADO
      const { error: payError } = await supabase.from("payments").insert([{
        gym_id: item.gym_id,
        amount: item.sale_price,
        paid_amount: item.sale_price,
        cost_basis: item.cost || 0, // GUARDAMOS EL COSTO AQUÍ
        method: "cash",
        status: "paid",
  ...
  return (
    <div className="grid gap-8">
      {/* Resumen de Ganancia REAL (Acumulativa) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <SummaryCard
          label="Ventas Totales (Caja)"
          value={"$" + new Intl.NumberFormat("es-AR").format(totalSales)}
          icon={DollarSign}
          sub="Plata total ingresada"
        />
        <SummaryCard
          label="Ganancia Neta Realizada"
          value={"$" + new Intl.NumberFormat("es-AR").format(realizedProfit)}
          icon={PiggyBank}
          tone="profit"
          sub="Beneficio real acumulado"
        />
        <SummaryCard
          label="Inversión Actual"
          value={"$" + new Intl.NumberFormat("es-AR").format(items.reduce((acc, i) => acc + ((i.cost || 0) * i.stock), 0))}
          icon={Boxes}
          sub="Plata en mercadería hoy"
        />
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className={cn(
              "group relative flex flex-col rounded-[2.5rem] border bg-card p-8 shadow-sm transition-all hover:shadow-2xl hover:border-primary/30",
              ...
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => handleDelete(item.id)} className="h-8 w-8 rounded-lg bg-red-500/10 text-red-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <div className="mt-6 space-y-4">
                ...
                <div className="flex justify-between items-center px-1">
                  <div>
                    ...
                    <p className="text-[8px] font-bold uppercase opacity-50">Stock Disponible</p>
                  </div>
              ...
              >
                {sellingId === item.id ? <Loader2 className="h-5 w-5 animate-spin" /> : "REGISTRAR VENTA"}
              </button>
            </div>
          );
        })}
...
function SummaryCard({ label, value, icon: Icon, tone, sub }: any) {
  return (
    <div className={cn(
      "p-8 rounded-[2.5rem] border bg-card shadow-lg flex items-center justify-between transition-all border-white/5",
      tone === 'profit' && "border-green-500/30 bg-green-500/[0.03]"
    )}>
      <div>
