...
export function InventoryList() {
  ...
  const fetchData = async () => {
    ...
    // 2. Obtener Pagos (Ventas reales de hoy en adelante)
    ...
    // RESET TOTAL: Si no hay pagos, forzamos 0
  ...
  return (
    <div className="grid gap-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <SummaryCard label="Ventas Totales (Caja)" value={"$" + new Intl.NumberFormat("es-AR").format(totalSales)} icon={DollarSign} sub="Plata ingresada" />
        <SummaryCard label="Ganancia Neta Realizada" value={"$" + new Intl.NumberFormat("es-AR").format(realizedProfit)} icon={PiggyBank} tone="profit" sub="Beneficio acumulado" />
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className={cn("group relative flex flex-col rounded-[2.5rem] border bg-card p-8 shadow-sm transition-all hover:shadow-2xl hover:border-primary/30", isLowStock && "border-red-500/20 bg-red-500/[0.02]")}>
              ...
              <button onClick={() => handleSell(item)} disabled={sellingId === item.id || item.stock <= 0} className={cn("mt-6 h-14 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95", item.stock > 0 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>
